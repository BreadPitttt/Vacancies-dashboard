# scraper.py — Final Hardened Version v3
# - Implements: All previous enhancements (Aggregator-first discovery, etc.)
# - NEW: Simplified fetch loop (relies on urllib3.Retry), enhanced PDF deadline probing with pdfplumber,
#   and circuit breaker state exposed in metrics.
...
OFFICIAL_DOMAINS = {
  ...
  "rrbcdg.gov.in", "bpsc.bihar.gov.in", "ssc.gov.in", "ssc.nic.in",
}
...
def session_with_retries(pool=96):
  ...
  if Retry:
    retry = Retry(
      ...
      status_forcelist=[429,500,502,503,504], # No 403
...
# Simplified fetch, relies on urllib3.Retry for backoff
def fetch(url, timeout, use_cache=False):
  ...
  try:
    ...
    if r.status_code == 403:
        print(f"[BLOCK] Received 403 from {url}. Halting retries.")
        r.raise_for_status()
    ...
    # If it fails even with retries, try Cloudflare solver once as a last resort
...
# ---------------- Parsing & Filters (largely unchanged) ----------------
# All parsing functions (norm, absolute, soup_from_resp, etc.) remain the same
...
def _decode_html_bytes(resp):
  ...
  return UnicodeDammit(resp.content, is_html=True, known_definite_encodings=[enc,"utf-8"]).unicode_markup
...
EXCLUDE_EDU = [r"\bmaster'?s\b|\bm\.?\s?sc\b|\bm\.?\s?a\b", r"\bmba\b", r"\b(b\.?tech|be)\b|\bm\.?tech\b", r"\bmca\b|\bbca\b", r"\blaw\b|\bllb\b", r"\bca\b|\bcfa\b", r"\bmbbs\b|\bbds\b|\bnursing\b", r"\bphd\b"]
ALLOW_SKILLS = [r"\btyping\b", r"\bsteno\b", r"\bccc\b|\bnielit\b|\bdoeacc\b", r"\bpet\b|\bpst\b"]
EXCLUDE_TECH = [r"\bpython\b|\bjava\b|\bjavascript\b", r"\bautocad\b|\bmatlab\b|\bsap\b"]
...
def probe_deadline_from_link(primary_url):
  try:
    ...
    if "html" in ct:
      s = soup_from_resp(r)
      if s:
        _, e = parse_application_window(norm(s.get_text(" ")[:2000]))
        return e
    elif pdfplumber and ("pdf" in ct or "pdf" in primary_url.lower()):
      with pdfplumber.open(io.BytesIO(r.content)) as pdf:
        if pdf.pages:
          first_page = pdf.pages[0]
          _, e = parse_application_window(first_page.extract_text())
          return e
...
def load_agg_seen():
  try:
    ...
    for k, info in data.items():
      kept=[(s, ts) for s, ts in info.get("seen", []) if datetime.fromisoformat(ts.replace("Z","")) >= cutoff]
      if kept: pruned[k] = {"seen": kept}
    return pruned
  except: return {}
def save_agg_seen(cache):
  try: AGG_CACHE_PATH.write_text(json.dumps(cache, indent=2), encoding="utf-8")
  except: pass

# ---------------- Main Scraping & Merge Logic (largely unchanged) ----------------
# The scrape_generic_list, scrape_telegram_channel, and merge_and_verify functions
# remain the same as the last version you received, as their logic is sound.
# They will now benefit from the more robust `fetch` function.
def scrape_generic_list(src, treat_as="aggregator", metrics=None):
  ...
  try:
    ...
    for title, href in anchors:
      ...
      rec = {
        "key": key_base, "uniqueKey": unique_key, "title": title, "organization": src["name"],
        "applyLink": official_link or href, "pdfLink": official_link or href, "deadline": deadline_here,
        ...
        "isUpdate": upd, "updateSummary": title if upd else None, "detailText": detail_text
      }
...
def scrape_telegram_channel(username, metrics):
  ...
  if not cb_before(key):
    metrics[key]["skipped_due_cb"] = metrics[key].get("skipped_due_cb", 0) + 1; return []
    ...
    for url in telegram_feed_urls(username):
      try:
        ...
        for it in root.findall(".//item"):
          title=tx(it,"title"); link=tx(it,"link"); desc=tx(it,"description"); combo=norm(f"{title} {desc}")
          if len(title)<6 or not is_joblike(combo) or is_non_vacancy(combo) or other_state_only(combo) or not education_allowed(combo): continue
          key_base=norm_key(title); upd=is_update(combo); unique_key=f"{key_base}|upd" if upd else key_base
          rec = {
            "key":key_base, "uniqueKey":unique_key, "title":title, "organization":f"Telegram:{username}",
            "applyLink":link, "pdfLink":link, "deadline":extract_deadline(combo, fallback_url=link),
            "domicile":"Bihar" if "bihar" in combo.lower() else "All India", "sourceType":"telegram",
            "source":f"Telegram:{username}", "isUpdate":upd, "updateSummary":title if upd else None, "detailText":combo
          }
          if validate_inline(rec): items.append(rec); kept+=1
        cb_succ(key); metrics[key]["ok"]=metrics[key].get("ok",0)+1; metrics[key]["durations"].append(time.time()-started); metrics[key]["kept"]=metrics[key].get("kept",0)+kept
        return items
      except Exception as e:
        metrics[key].setdefault("error_samples",[]).append(str(e)); continue
  except:
...
def merge_and_verify(primary_items, backup_items, official_items):
  ...
  def add(it):
    ...
    if it["sourceType"]=="official": b["hasOfficial"]=True; b["offNames"].add(it["source"])
    b["items"].append(it)
  for lst in (primary_items, backup_items, official_items):
    for it in lst: add(it)
  agg_seen = load_agg_seen(); published=[]; pending=set(); seen_ids=set()
  for key, b in buckets.items():
    rep=None; verifiedBy=None
    if b["hasOfficial"]:
      rep=next((x for x in b["items"] if x.get("sourceType")=="official" and not x.get("isUpdate")),None) or next((x for x in b["items"] if x.get("sourceType")=="official"),None)
      verifiedBy="official"
    else:
      historical=set([s for s,_ in agg_seen.get(key,{}).get("seen",[])])
      combined = set(b["aggs"]) | historical
      if len(combined) >= 2:
        rep=next((x for x in b["items"] if x.get("sourceType")=="aggregator" and not x.get("isUpdate")),None) or next((x for x in b["items"] if x.get("sourceType")=="aggregator"),None)
        verifiedBy="multi-aggregator"
      else: pending.add(key); continue
    if not rep: continue
    dl=rep.get("deadline"); ok_deadline=False
    if dl:
      try: ok_deadline = dateparser.parse(dl, settings={"DATE_ORDER":"DMY"}).date() >= datetime.now().date()
      except: ok_deadline=False
    ...
    rid=base_id; n=1
    while rid in seen_ids: rid=hashlib.sha1(f"{base_id}|{n}".encode("utf-8")).hexdigest()[:12]; n+=1
    seen_ids.add(rid)
    rep_rec = {
      "id":rid, "slug":rid, "title":rep["title"], "organization":"/".join(sources) or rep["organization"],
      "qualificationLevel":infer_qualification(rep["detailText"]), "domicile":rep["domicile"], "source":schema_source,
      "verifiedBy":verifiedBy, "type":"VACANCY", "updateSummary":None, "relatedTo":None, "deadline":rep.get("deadline"),
    ...
    for u in (x for x in b["items"] if x.get("isUpdate")):
      ...
      uid=up_base; m=1
      while uid in seen_ids: uid=hashlib.sha1(f"{up_base}|{m}".encode("utf-8")).hexdigest()[:12]; m+=1
      seen_ids.add(uid)
      upd = {
        "id":uid, "slug":uid, "title":"[UPDATE] "+u["title"], "organization":rep_rec["organization"],
        "qualificationLevel":infer_qualification(u["detailText"]), "domicile":rep["domicile"], "source":schema_source,
        "verifiedBy":verifiedBy, "type":"UPDATE", "updateSummary":u.get("updateSummary"), "relatedTo":rep_rec["slug"],
        "deadline":u.get("deadline") or rep_rec["deadline"], "applyLink":u.get("applyLink") or rep_rec["applyLink"],
        "pdfLink":u.get("pdfLink") or rep_rec["pdfLink"], "extractedAt":UTC_NOW.strftime("%Y-%m-%dT%H:%M:%SZ"),
      }
...
def main():
  ...
  per_source_metrics = {}
  all_sources = PRIMARY_AGG+BACKUP_AGG+OFFICIAL_SOURCES
  for s in all_sources: per_source_metrics[s["name"]]={"ok":0,"fail":0,"skipped_due_cb":0,"not_modified":0,"raw":0,"hinted":0,"kept":0,"error_samples":[],"durations":[],"cb_state":"closed"}
  for ch in TELEGRAM_CHANNELS: per_source_metrics[f"TG:{ch}"]={"ok":0,"fail":0,"skipped_due_cb":0,"not_modified":0,"raw":0,"hinted":0,"kept":0,"error_samples":[],"durations":[],"cb_state":"closed"}
  ...
  with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
    ...
    for f in as_completed(futs):
      ...
      try:
        ...
        print(f"[ERROR] {tag}:{name} thread failed: {e}")
  ...
  for it in primary_items + backup_items: note_agg(it)
  save_agg_seen(agg_seen)

  published, pending = merge_and_verify(primary_items, backup_items, official_items)

  prev = load_data(); data = prev; data["jobListings"] = published
  ...
  ti["aggCounts"] = agg_counts; ti["officialCounts"] = off_counts; ti["telegramCounts"] = tg_counts
  ti["pendingFromAggregators"] = sorted(list(pending))
  ti["notes"] = "v3: Agg-first, official-verify, rolling corroboration, PDF probing, UA rotation, proxy support."
  ti["runDurationSec"] = round(duration,2)
  # Add final CB state to metrics
  for name in per_source_metrics: per_source_metrics[name]["cb_state"] = get_cb_state(name)
