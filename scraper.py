# scraper.py — modernized for speed and stability:
# - Stable IDs (SHA-1), retries + shorter timeouts (connect/read tuples),
# - Correct decoding via UnicodeDammit(known_definite_encodings),
# - Skip non-HTML, cap per source, concurrent detail fetch with thread-local Sessions.
...
HEAD_TO    = 8
MAX_WORKERS = 8
PER_SOURCE_MAX = 120  # upper bound to keep runs predictable
...
def fetch(url, timeout):
  r = HTTP.get(url, timeout=timeout)  # tuple timeouts for connect/read [24]
  r.raise_for_status()
  return r

def decode_html_bytes(resp):
  enc = resp.encoding or getattr(resp, "apparent_encoding", None) or "utf-8"
  # Guide BeautifulSoup to use the likely encodings to avoid long guesses + U+FFFD [3][4][5]
...
def scrape_html_list(list_url, base, org):
  ...
  try:
    ...
    def fetch_detail(pair):
      ...
      if not (passes_education(combo) and passes_skill_rule(combo) and passes_degree_exclusions(combo) and is_recruitment(combo) and eligible_by_domicile(org, detail_text)):
        return None
