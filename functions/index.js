...
export default {
  async fetch(req, env, ctx) {
    ...
    if (type === "vote") {
      ...
      const rec = { type, vote: body.vote, jobId: String(body.jobId||"").trim(), title: String(body.title||"").trim(), url: normalizeUrl(String(body.url||"").trim()), ts: new Date().toISOString() };
    ...
    if (type === "state") {
      ...
      const jobId = String((p.jobId||"")).trim(), action=String((p.action||"")).trim();
