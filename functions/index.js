...
if (req.method !== "POST") return cors(new Response("Method Not Allowed", { status: 405 }), ALLOW_ORIGIN);
if (origin !== ALLOW_ORIGIN) return cors(json({ error: "Origin not allowed" }, 403), ALLOW_ORIGIN);
...
s=String(s).trim();
let m=s.match(/^(\d{1,2})-/-/$/);
if(m){ const d=m.padStart(2,"0"), mo=m.padStart(2,"0"), y=m; return ${d}/${mo}/${y}; }
m=s.match(/^(\d{1,2})-/-/$/);
if(m){ const d=m.padStart(2,"0"), mo=m.padStart(2,"0"), y=20${m}; return ${d}/${mo}/${y}; }
return s;
}
