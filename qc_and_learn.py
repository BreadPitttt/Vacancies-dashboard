...
parents = [j for j in listings if not is_update_title(j.get("title"))]
...
# Accept official-like missing submissions and learn captureHints
OFFICIAL_WHITELIST = {"ssc.gov.in","upsconline.gov.in","upsconline.nic.in","ibps.in","bpsc.bih.nic.in","bpsc.bihar.gov.in","opportunities.rbi.org.in","dsssb.delhi.gov.in","bssc.bihar.gov.in","onlinebssc.com","rrbcdg.gov.in","rrbapply.gov.in","ccras.nic.in"}
added_from_missing = 0
for s in subs:
    if s.get("type")=="missing" and s.get("url") and s.get("title"):
        u = norm_url(s["url"]); dom = urllib.parse.urlparse(u).hostname or ""
